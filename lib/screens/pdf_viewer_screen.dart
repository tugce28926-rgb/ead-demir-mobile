...
class _PdfViewerScreenState extends State<PdfViewerScreen> {
  ...
  Future<void> _loadDocumentPdf() async {
    ...
    try {
      ...
      if (bytes.isEmpty) {
        setState(() {
          _error = 'Belge bulunamadı veya oluşturulamadı.';
          _isLoading = false;
        });
        return;
      }
  ...
  Widget build(BuildContext context) {
    final docTitle = widget.documentNo.isNotEmpty
        ? widget.documentNo
        : (widget.type == 'fatura' ? 'e-Fatura' : 'e-İrsaliye');
              ...
              onPressed: () async {
                try {
                  await Printing.sharePdf(
                      bytes: _pdfBytes!, filename: '${widget.type}_${widget.documentNo}.pdf');
                } catch (e) {
                  if (context.mounted) {
          ...
          ? const Center(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 14),
                  Text('GİB Belgesi Hazırlanıyor...',
                      style: TextStyle(color: AppTheme.slate600, fontSize: 13, fontWeight: FontWeight.w600)),
                ],
              ),
            )
          : _error != null
